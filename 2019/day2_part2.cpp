...
using PointSet = std::unordered_map<Point, unsigned int, pair_hash>;

PointSet buildPath(std::string path)
{
    ...
    for (;;)
    {
        ...
        assert(iss);
        instructions.push_back({direction, width});
    ...
    Point p{0, 0};
    PointSet visitedCells;
    unsigned int step = 1;
    for (const Instruction &instruction : instructions)
    {
        for (unsigned int i = 0; i < instruction.width; i++)
        {
...
void solve(const std::string &firstPath,
           const std::string &secondPath,
           unsigned const expectedResult)
{
    PointSet pathA = buildPath(firstPath);
    PointSet pathB = buildPath(secondPath);

    std::optional<unsigned int> minDistance;
    for (const auto &cellA : pathA)
    {
        const auto maybeIntersect = pathB.find(cellA.first);
        if (maybeIntersect != pathB.end())
        {
            unsigned int dist = cellA.second + maybeIntersect->second;
            if (!minDistance.has_value() || dist < minDistance.value())
            {
                minDistance = dist;
            }
        }
    }
    std::cout << minDistance.value();
    if (minDistance.value() != expectedResult)
    {
        std::cout << " (wrong)";
    }
    std::cout << "\n";
